...
### query aufbauen
query <- list(page = 1, name = "Lauren Boebert", api_key = fec_key,
              election_year = 2020, office = "H")
...
lauren_boebert_fec <- httr::GET(fec_url)
```
...
# Endpunkt der API bestimmen (URL)
candidate_id_df %>%
  filter(name == 'GAETZ, MATT') 

# die einzelnen Parameter übergeben wir als Objekte einer Liste
query_indep_exp <- list(cycle = 2020, api_key = fec_key,
                        election_full = "true", page = 1, per_page = 100,
                        candidate_id = "H6FL01119")
```

```{r}
# Aufbau der URL
fec_inde_exp_url <- httr::modify_url(url = indep_expend_endpoint, 
                                     query = query_indep_exp)

# Ziehen der Informationen
matt_gaetz_indep_exp <- httr::GET(fec_inde_exp_url)
```

```{r}
# Daten für R lesbar machen und zu Data frame umwandeln
matt_gaetz_indep_exp %>%
    httr::content(as = "text", encoding = "UTF-8") %>%
    jsonlite::fromJSON() %>% 
    purrr::pluck("results")
```
...
final_df %>% 
  filter(support_oppose_indicator == "S") %>% 
  group_by(candidate_id, name) %>% 
  summarise(total_money = sum(total)) %>% 
...
```{r}
POST(jokes_url, body = joke_body, encode = "json", verbose())
```
