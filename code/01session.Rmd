...
- *Titel*: /html/body/div[1]/div[2]/div[1]/div[2]/div/div/div/div/div/div[3]/div/div/div/div[3]/div[1]/div[2]/article/div[2]/header/h3/a
...
Das geht jetzt natürlich nicht nur für die Titel, wir wollen auch noch Datum und Uhrzeit, sowie den hinterlegten Event-Link für jede der angezeigten Veranstaltungen:
...
purrr::map(.x = pages, ~ {
  ...
  polite::nod(session, str_c('?post_type=tribe_events&eventDisplay=list&paged=', .x)) %>% 
  #... und die HTML scrapen!
    polite::scrape() 
}) -> results   

# Alternativ zum lokalen Abspeichern der Dateien: events//liste/?tribe_event_display=past&tribe_paged='
purrr::map(.x = pages, ~ {
  #...die URL für jedes Jahr zusammenbasteln und uns jeweils auf der Seite anmelden...
  polite::nod(session, str_c('?post_type=tribe_events&eventDisplay=list&paged=', .x)) %>% 
  #...und abspeichern!
    polite::rip(destfile = str_c('events_', .x, '.html'),
                path = here::here('assets', 'htmls'),
                overwrite = FALSE) # wenn ihr die Dateien überschreiben wollt, dann zu TRUE ändern!
})
```
